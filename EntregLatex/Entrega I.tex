...
\node(StartApp){\includegraphics[width=1.3cm]{./Images/stickman}};
\node[cas, right = of StartApp, xshift=2cm](Create){Crear\\partida};
...
\node[cas, above right = of Create](CreateUser){Creada\\per l'usuari};
\node[cas, right = of Create](CreateMachine){Creada per\\la màquina};
\node[cas, below = of Create](Solve){Resoldre\\partida};
